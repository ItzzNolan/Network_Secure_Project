...
def cmd_run(args):
    ...
    py_port = args.py_port if hasattr(args, "py_port") else 9998

    if ais:
        partie = initialiser(ais, config["units"], map_size=map_size, py_port=py_port)
    else:
        partie = Jeu(largeur=map_size, hauteur=map_size, py_port=py_port)

    # Initialisation Réseau
    if hasattr(args, "network") and args.network:
        partie.player_id = args.player_id if hasattr(args, "player_id") else 0
        partie.network_mode = True
        partie.next_unit_id = partie.player_id * 10000  # IDs uniques par joueur
