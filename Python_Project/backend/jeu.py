...
class Jeu:
    ...
    def del_unit_by_id(self, uid):
        for u in self.unites:
            if getattr(u, 'id', -1) == uid: u.alive, u.HP,u = False, 0,None
        self.unites = [u for u in self.unites if u is not None]
            ...
            for a in acts:
                ...
                if a.type in [TypeAction.MOVE, TypeAction.FORM_UP]:
                    ...
                    if dist > 0.1:
                        ...
                        self.ipc.envoyer({"type":"UPDATE","action":"move","entity_id":u.id,"x":u.coords[0],"y":u.coords[1],"player_id":u.equipe,"unit_type":getattr(u, 'Unit', 'Knight'),"hp":u.HP})
                elif a.type == TypeAction.ATTACK:
    ...
    def appliquer_message(self, m):
        ...
        if t == "join":
            if pid not in self.generaux:
                ...
                entities = [{"entity_id": u.id, "owner_id": u.equipe, "unit_type": getattr(u, 'Unit', 'Knight'), "x": u.coords[0], "y": u.coords[1], "hp": u.HP} for u in self.unites if u.alive]
            ...
            for ent in m.get("entities", []):
                ...
                eq = ent.get("owner_id")
                if eq not in self.generaux and eq != self.player_id:
                    self.generaux[eq] = make_general("braindead", id_player=eq)
                
                u = self.get_unit_by_id(eid)
                if u:
                    u.coords = (ent.get("x"), ent.get("y"))
                    u.HP = ent.get("hp")
                else:
                    self.ajouter_unite(ent.get("unit_type"), ent.get("x"), ent.get("y"), eq, force_id=eid)
                    nu = self.get_unit_by_id(eid)
                    if nu: nu.HP = ent.get("hp")

        elif t == "update":
            act = m.get("action", "").lower()
            if act == "move":
                self.del_unit_by_id(m.get("entity_id"))
                self.ajouter_unite(m.get("unit_type"), m.get("x"), m.get("y"), m.get("player_id"), force_id=m.get("entity_id"))
                u = self.get_unit_by_id(m.get("entity_id"))
                if u: u.HP = m.get("hp", u.HP)
            elif act == "attack":
            ...
            if pid in self.generaux: del self.generaux[pid]
            for u in self.unites:
                if getattr(u, 'equipe', -1) == pid:
                    u.alive, u.HP = False, 0

    def disconnect(self): self.ipc.envoyer({"type": "DISCONNECT", "player_id": self.player_id})
    def check_victory(self):
        if len(self.generaux) < 2: return None
        alive_by_team = {}
        for u in self.unites:
            if u and getattr(u, "alive", False):
                team = getattr(u, "equipe", None)
                if team is not None: alive_by_team[team] = alive_by_team.get(team, 0) + 1
    ...
    def distance_tiles(self, p1, p2): return abs(p1[0]-p2[0]) + abs(p1[1]-p2[1])
    def all_seen_enemies(self, pid): return [u for u in self.unites if u.alive and u.equipe != pid]
    def all_seen_allies(self, pid): return [u for u in self.unites if u.alive and u.equipe == pid]
    def is_walkable(self, pos): return self.carte.est_dans_grille(int(pos[0]), int(pos[1])) if pos else False
