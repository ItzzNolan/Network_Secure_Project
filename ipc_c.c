...
#include <sys/select.h> // Pour la surveillance multiple

#define PORT_C 9999      // Port où le C écoute Python
#define PORT_PY 9998     // Port où Python écoute le C
#define BUF_SIZE 2048

// Fonction pour envoyer un message JSON au processus Python
...
// Fonction d'analyse du protocole 
void interpreter_msg_python(char *raw_json) {
    ...
    if (cJSON_IsString(type)) {
        if (strcmp(type->valuestring, "UPDATE") == 0) {
            ...
            // Ici, on appellera la fonction de diffusion de Léo (C1)
        }
    }
    cJSON_Delete(json);
}
...
int main() {
    ...
    // Configuration de l'adresse de destination (Python)
    ...
    while(1) {
        ...
        // On attend que quelque chose arrive sur le socket
        // (En situation réelle, on ajouterait ici le socket réseau de Léo)
        ...
        if (activity > 0 && FD_ISSET(sockfd, &readfds)) {
            ...
            // Exemple : On renvoie un accusé de réception à Python pour tester
